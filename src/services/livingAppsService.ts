...
import type { Speisekarte, Oeffnungszeiten, Galerie, StandortKontakt, CreateSpeisekarte, CreateOeffnungszeiten, CreateGalerie, CreateStandortKontakt } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getSpeisekarte(): Promise<Speisekarte[]> {
    ...
    return hydrateRecords(records, 'speisekarte');
  }
  static async getSpeisekarteEntry(id: string): Promise<Speisekarte | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Speisekarte;
    return hydrateRecords([record], 'speisekarte')[0];
  }
  static async createSpeisekarteEntry(fields: CreateSpeisekarte): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SPEISEKARTE}/records`, { fields: cleanFieldsForApi(fields as any, 'speisekarte') });
    return { ...data, record_id: data.id };
  }
  static async updateSpeisekarteEntry(id: string, fields: Partial<CreateSpeisekarte>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SPEISEKARTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'speisekarte') });
    return { ...data, record_id: data.id };
  }
  ...
  // --- OEFFNUNGSZEITEN ---
  static async getOeffnungszeiten(): Promise<Oeffnungszeiten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.OEFFNUNGSZEITEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Oeffnungszeiten[];
    return hydrateRecords(records, 'oeffnungszeiten');
  }
  static async getOeffnungszeitenEntry(id: string): Promise<Oeffnungszeiten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.OEFFNUNGSZEITEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Oeffnungszeiten;
    return hydrateRecords([record], 'oeffnungszeiten')[0];
  }
  static async createOeffnungszeitenEntry(fields: CreateOeffnungszeiten): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.OEFFNUNGSZEITEN}/records`, { fields: cleanFieldsForApi(fields as any, 'oeffnungszeiten') });
    return { ...data, record_id: data.id };
  }
  static async updateOeffnungszeitenEntry(id: string, fields: Partial<CreateOeffnungszeiten>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.OEFFNUNGSZEITEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'oeffnungszeiten') });
    return { ...data, record_id: data.id };
  }
  static async deleteOeffnungszeitenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.OEFFNUNGSZEITEN}/records/${id}`);
  }

  // --- GALERIE ---
  static async getGalerie(): Promise<Galerie[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GALERIE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Galerie[];
    return hydrateRecords(records, 'galerie');
  }
  static async getGalerieEntry(id: string): Promise<Galerie | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.GALERIE}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Galerie;
    return hydrateRecords([record], 'galerie')[0];
  }
  static async createGalerieEntry(fields: CreateGalerie): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.GALERIE}/records`, { fields: cleanFieldsForApi(fields as any, 'galerie') });
    return { ...data, record_id: data.id };
  }
  static async updateGalerieEntry(id: string, fields: Partial<CreateGalerie>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.GALERIE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'galerie') });
    return { ...data, record_id: data.id };
  }
  static async deleteGalerieEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.GALERIE}/records/${id}`);
  }

  // --- STANDORT_KONTAKT ---
  static async getStandortKontakt(): Promise<StandortKontakt[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.STANDORT_KONTAKT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as StandortKontakt[];
    return hydrateRecords(records, 'standort_kontakt');
  }
  static async getStandortKontaktEntry(id: string): Promise<StandortKontakt | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.STANDORT_KONTAKT}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as StandortKontakt;
    return hydrateRecords([record], 'standort_kontakt')[0];
  }
  static async createStandortKontaktEntry(fields: CreateStandortKontakt): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.STANDORT_KONTAKT}/records`, { fields: cleanFieldsForApi(fields as any, 'standort_kontakt') });
    return { ...data, record_id: data.id };
  }
  static async updateStandortKontaktEntry(id: string, fields: Partial<CreateStandortKontakt>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.STANDORT_KONTAKT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'standort_kontakt') });
    return { ...data, record_id: data.id };
  }
  static async deleteStandortKontaktEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.STANDORT_KONTAKT}/records/${id}`);
  }
