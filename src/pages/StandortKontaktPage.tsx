...
export default function StandortKontaktPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<StandortKontakt[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<StandortKontakt | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<StandortKontakt | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getStandortKontakt());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: StandortKontakt['fields']) {
  ...
  async function handleUpdate(fields: StandortKontakt['fields']) {
    ...
    await LivingAppsService.updateStandortKontaktEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteStandortKontaktEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title={appLabel('standort_kontakt')}
      subtitle={`${records.length} ${t('in_system', { entity: appLabel('standort_kontakt') })}`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder={t('search_entity', { entity: appLabel('standort_kontakt') })}
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
            ...
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('restaurant_name')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'restaurant_name')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('strasse')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'strasse')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('hausnummer')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'hausnummer')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('postleitzahl')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'postleitzahl')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('ort')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'ort')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('telefon')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'telefon')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('email')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'email')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('webseite')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'webseite')}
                  {sortKey === 'webseite' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('reservierungshinweis')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'reservierungshinweis')}
                  {sortKey === 'reservierungshinweis' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('standort_karte')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('standort_kontakt', 'standort_karte')}
                  {sortKey === 'standort_karte' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/standort-kontakt/${record.record_id}`); }}>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.reservierungshinweis ?? '—'}</span></TableCell>
                <TableCell className="max-w-[200px]"><span className="truncate block" title={record.fields.standort_karte ? `${record.fields.standort_karte.lat}, ${record.fields.standort_karte.long}` : undefined}>{record.fields.standort_karte?.info ?? (record.fields.standort_karte ? `${record.fields.standort_karte.lat?.toFixed(4)}, ${record.fields.standort_karte.long?.toFixed(4)}` : '—')}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? t('no_results') : t('no_data_yet', { entity: appLabel('standort_kontakt') })}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
