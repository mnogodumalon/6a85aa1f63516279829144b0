...
import { OeffnungszeitenViewDialog } from '@/components/dialogs/OeffnungszeitenViewDialog';
import { GalerieDialog } from '@/components/dialogs/GalerieDialog';
import { GalerieViewDialog } from '@/components/dialogs/GalerieViewDialog';
import { StandortKontaktDialog } from '@/components/dialogs/StandortKontaktDialog';
import { StandortKontaktViewDialog } from '@/components/dialogs/StandortKontaktViewDialog';
...
import { t, appLabel, fieldLabels, lookupLabel } from '@/i18n';
...
const SPEISEKARTE_FIELDS = [
  { key: 'gericht_name', label: 'Name des Gerichts', type: 'string/text' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'vorspeise', label: 'Vorspeise' }, { key: 'suppe', label: 'Suppe' }, { key: 'hauptgericht', label: 'Hauptgericht' }, { key: 'pasta', label: 'Pasta' }, { key: 'pizza', label: 'Pizza' }, { key: 'salat', label: 'Salat' }, { key: 'dessert', label: 'Dessert' }, { key: 'getraenk', label: 'Getränk' }, { key: 'tagesgericht', label: 'Tagesgericht' }] },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'preis', label: 'Preis (€)', type: 'number' },
  { key: 'gericht_foto', label: 'Foto des Gerichts', type: 'file' },
  { key: 'vegetarisch', label: 'Vegetarisch', type: 'bool' },
  { key: 'vegan', label: 'Vegan', type: 'bool' },
  { key: 'glutenfrei', label: 'Glutenfrei', type: 'bool' },
];
const OEFFNUNGSZEITEN_FIELDS = [
  ...
  { key: 'geschlossen', label: 'An diesem Tag geschlossen', type: 'bool' },
  { key: 'oeffnung_von', label: 'Geöffnet ab', type: 'string/text' },
  { key: 'oeffnung_bis', label: 'Geöffnet bis', type: 'string/text' },
  { key: 'mittagspause_von', label: 'Mittagspause von', type: 'string/text' },
  { key: 'mittagspause_bis', label: 'Mittagspause bis', type: 'string/text' },
  { key: 'hinweis_tag', label: 'Hinweis zum Tag', type: 'string/text' },
];
const GALERIE_FIELDS = [
  { key: 'foto_titel', label: 'Titel des Fotos', type: 'string/text' },
  { key: 'foto_datei', label: 'Foto', type: 'file' },
  { key: 'foto_beschreibung', label: 'Bildbeschreibung', type: 'string/textarea' },
  { key: 'foto_kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'innenraum', label: 'Innenraum' }, { key: 'aussenbereich', label: 'Außenbereich' }, { key: 'gerichte', label: 'Gerichte' }, { key: 'team', label: 'Team' }, { key: 'atmosphaere', label: 'Atmosphäre' }, { key: 'sonstiges', label: 'Sonstiges' }] },
];
const STANDORTKONTAKT_FIELDS = [
  { key: 'restaurant_name', label: 'Name des Restaurants', type: 'string/text' },
  ...
  { key: 'reservierungshinweis', label: 'Hinweis zur Reservierung', type: 'string/textarea' },
  { key: 'standort_karte', label: 'Standort auf der Karte', type: 'geo' },
];

const ENTITY_TABS = [
  { key: 'speisekarte', pascal: 'Speisekarte' },
  { key: 'oeffnungszeiten', pascal: 'Oeffnungszeiten' },
  { key: 'galerie', pascal: 'Galerie' },
  { key: 'standort_kontakt', pascal: 'StandortKontakt' },
] as const;
...
export default function AdminPage() {
  ...
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'speisekarte': new Set(),
    'oeffnungszeiten': new Set(),
    'galerie': new Set(),
    'standort_kontakt': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'oeffnungszeiten': return (data as any).oeffnungszeiten as Oeffnungszeiten[] ?? [];
      case 'galerie': return (data as any).galerie as Galerie[] ?? [];
      case 'standort_kontakt': return (data as any).standortKontakt as StandortKontakt[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        case 'speisekarte': return SPEISEKARTE_FIELDS as any[];
        case 'oeffnungszeiten': return OEFFNUNGSZEITEN_FIELDS as any[];
        case 'galerie': return GALERIE_FIELDS as any[];
        case 'standort_kontakt': return STANDORTKONTAKT_FIELDS as any[];
        default: return [];
      }
    })();
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'oeffnungszeiten': return {
        create: (fields: any) => LivingAppsService.createOeffnungszeitenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateOeffnungszeitenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteOeffnungszeitenEntry(id),
      };
      case 'galerie': return {
        create: (fields: any) => LivingAppsService.createGalerieEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateGalerieEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteGalerieEntry(id),
      };
      case 'standort_kontakt': return {
        create: (fields: any) => LivingAppsService.createStandortKontaktEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateStandortKontaktEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteStandortKontaktEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'oeffnungszeiten' || dialogState?.entity === 'oeffnungszeiten') && (
        <OeffnungszeitenDialog
          open={createEntity === 'oeffnungszeiten' || dialogState?.entity === 'oeffnungszeiten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'oeffnungszeiten' ? handleUpdate : (fields: any) => handleCreate('oeffnungszeiten', fields)}
          defaultValues={dialogState?.entity === 'oeffnungszeiten' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Oeffnungszeiten']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Oeffnungszeiten']}
        />
      )}
      {(createEntity === 'galerie' || dialogState?.entity === 'galerie') && (
        <GalerieDialog
          open={createEntity === 'galerie' || dialogState?.entity === 'galerie'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'galerie' ? handleUpdate : (fields: any) => handleCreate('galerie', fields)}
          defaultValues={dialogState?.entity === 'galerie' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Galerie']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Galerie']}
        />
      )}
      {(createEntity === 'standort_kontakt' || dialogState?.entity === 'standort_kontakt') && (
        <StandortKontaktDialog
          open={createEntity === 'standort_kontakt' || dialogState?.entity === 'standort_kontakt'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'standort_kontakt' ? handleUpdate : (fields: any) => handleCreate('standort_kontakt', fields)}
          defaultValues={dialogState?.entity === 'standort_kontakt' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'oeffnungszeiten' && (
        <OeffnungszeitenViewDialog
          open={viewState?.entity === 'oeffnungszeiten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'oeffnungszeiten', record: r }); }}
        />
      )}
      {viewState?.entity === 'galerie' && (
        <GalerieViewDialog
          open={viewState?.entity === 'galerie'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'galerie', record: r }); }}
        />
      )}
      {viewState?.entity === 'standort_kontakt' && (
        <StandortKontaktViewDialog
          open={viewState?.entity === 'standort_kontakt'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'standort_kontakt', record: r }); }}
        />
      )}
