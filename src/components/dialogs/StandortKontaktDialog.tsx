/**
 * StandortKontaktDialog — pre-generated create/edit dialog for StandortKontakt.
 *
 ...
 * StandortKontaktDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<StandortKontakt['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<StandortKontaktDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/StandortKontakt';
...
/** Widened prefill type for StandortKontaktDialog.defaultValues — see file header. */
export type StandortKontaktDialogDefaults = StandortKontakt['fields'];

interface StandortKontaktDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: StandortKontakt['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: StandortKontaktDialogDefaults;
...
export function StandortKontaktDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: StandortKontaktDialogProps) {
  const [fields, setFields] = useState<Partial<StandortKontakt['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['restaurant_name', 'strasse', 'hausnummer', 'postleitzahl', 'ort', 'telefon'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<StandortKontakt['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'standort_kontakt');
      await onSubmit(clean as StandortKontakt['fields']);
      onClose();
    } catch (err) {
  ...
  // which: {"road": "strasse", "houseNumber": "hausnummer", "postcode": "postleitzahl", "city": "ort"}. The marker is the source of truth, so a
  // move OVERWRITES these fields (info always; components when Nominatim returns them).
  const ADDRESS_FIELD_MAP: Record<string, string | undefined> = {"road": "strasse", "houseNumber": "hausnummer", "postcode": "postleitzahl", "city": "ort"};
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<StandortKontakt['fields']>;
      });
      if (gps) {
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('standort_kontakt') })
    : t('new_entity', { entity: appLabel('standort_kontakt') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'restaurant_name': (
      <div key="restaurant_name" className="space-y-1.5">
        <Label htmlFor="restaurant_name">{fieldLabel('standort_kontakt', 'restaurant_name')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="restaurant_name"
          placeholder="z. B. La Cucina"
          value={fields.restaurant_name ?? ''}
          onChange={e => setFields(f => ({ ...f, restaurant_name: e.target.value }))}
          required
        />
        {showErrors && !fields.restaurant_name && (
    ...
    'strasse': (
      <div key="strasse" className="space-y-1.5">
        <Label htmlFor="strasse">{fieldLabel('standort_kontakt', 'strasse')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="strasse"
          placeholder="z. B. Via Roma"
    ...
    'hausnummer': (
      <div key="hausnummer" className="space-y-1.5">
        <Label htmlFor="hausnummer">{fieldLabel('standort_kontakt', 'hausnummer')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="hausnummer"
          placeholder="z. B. 42"
    ...
    'postleitzahl': (
      <div key="postleitzahl" className="space-y-1.5">
        <Label htmlFor="postleitzahl">{fieldLabel('standort_kontakt', 'postleitzahl')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="postleitzahl"
    ...
    'ort': (
      <div key="ort" className="space-y-1.5">
        <Label htmlFor="ort">{fieldLabel('standort_kontakt', 'ort')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="ort"
    ...
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('standort_kontakt', 'telefon')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="telefon"
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('standort_kontakt', 'email')}</Label>
        <Input
          id="email"
          type="email"
          placeholder="z. B. kontakt@lacucina.de"
          value={fields.email ?? ''}
    ...
    'webseite': (
      <div key="webseite" className="space-y-1.5">
        <Label htmlFor="webseite">{fieldLabel('standort_kontakt', 'webseite')}</Label>
        <Input
          id="webseite"
    ...
    'reservierungshinweis': (
      <div key="reservierungshinweis" className="space-y-1.5">
        <Label htmlFor="reservierungshinweis">{fieldLabel('standort_kontakt', 'reservierungshinweis')}</Label>
        <Textarea
          id="reservierungshinweis"
          placeholder="Reservierungsbedingungen, Öffnungszeiten, Besonderheiten..."
          value={fields.reservierungshinweis ?? ''}
          onChange={e => setFields(f => ({ ...f, reservierungshinweis: e.target.value }))}
          rows={3}
        />
      </div>
    ),
    'standort_karte': (
      <div key="standort_karte" className="space-y-1.5">
        <Label htmlFor="standort_karte">{fieldLabel('standort_kontakt', 'standort_karte')}</Label>
  ...
  const FIELD_LABELS: Record<string, string> = {"restaurant_name": "Name des Restaurants", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "ort": "Ort", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "webseite": "Webseite", "reservierungshinweis": "Hinweis zur Reservierung", "standort_karte": "Standort auf der Karte"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.STANDORT_KONTAKT} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
